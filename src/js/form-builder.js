...
const FormBuilder = function(opts, element) {
  ...
  let controlIndex = 0;
  let initObject = $('<div/>', {'class': 'form-build-tab-item active'});
  ...
  initObject.appendTo($cbUL);
  let tabList = {
	'baseFields': ['basic', false],
	'customFields': ['custom', true],
	'inputSets': ['set', true]
  };

	$.each(tabList, (key, info) => {
		let classTag = info[0];
		if (opts[key].length) {
			let divObject = $('<div/>', {'class': 'form-build-tab-item'});
			opts[key].forEach((set, i) => {
				set.name = set.name || utils.makeClassName(set.label);
				let inputSet = m('li', set.label, {
					className: 'input-' + classTag + '-control input-' + classTag + '-${i}',
					type: set.name
				});
				$(inputSet).appendTo(divObject);
			});
			divObject.appendTo($cbUL);
		}
	});
  ...
  let processControl = control => {
	let tag = true;
	let classList = control[0].classList;
	$.each(tabList, (key, info) => {
		let className = 'input-' + info[0] + '-control';
		let actionTag = info[1];
		if (classList.contains(className)) {
			tag = false;
			let inputSets = [];
			let inputSet = opts[key].find(set =>
			...
			inputSets.forEach(field => {
				prepFieldVars(field, true, actionTag);
  ...
  let prepFieldVars = function($field, isNew = false, actionTag = true) {
    let field = {};
    if ($field instanceof jQuery) {
      ...
      if (field.type) {
		...
		// field.api = controlClass.api(field.type);
    ...
    appendNewField(field, isNew, actionTag);
